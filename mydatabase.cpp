...
//打开连接
bool mydatabase::connectSql(const QString &dbName) {
    mdbName_= dbName;
...
//打开连接
bool mydatabase::connectSql(const QString &host, int port, const QString &dbName, const QString &userName, const QString &password) {
    mhost_= host;
    mport_= port;
    mdbName_= dbName;
    muser_= userName;
...
//错误打印
void mydatabase::errorSql(QString sql) {
    errorSqlText = sql;
    qCritical("%s", qPrintable(errorSqlText));
}

//获取错误的数据库语句
QString mydatabase::getErrorSql() {
...
void mydatabase::setMdbName(const QString &mdbName)
{
    mdbName_ = mdbName;
}

void mydatabase::setMpwd(const QString &mpwd)
{
    mpwd_ = mpwd;
}

void mydatabase::setMuser(const QString &muser)
{
    muser_ = muser;
}

void mydatabase::setMhost(const QString &mhost)
{
    mhost_ = mhost;
}

void mydatabase::setMport(int mport)
{
    mport_ = mport;
}

//执行sql语句，不获取结果
bool mydatabase::queryExec( QString queryStr) {
    if(connectionName.isEmpty()) {
...
//执行sql语句，并获取结果
bool mydatabase::queryExec( QString queryStr, QList<QHash<QString, QString>> &data) {
    data.clear();
    if(connectionName.isEmpty()) {
        if(!connectSql(mhost_,mport_,mdbName_,muser_,mpwd_)) {
            return false;
...
//获取数据
bool mydatabase::getData(QString tableName, QHash<QString, QString> &data, QString sqlWhere) {
    data.clear();
...
//获取数据
bool mydatabase::getData( QString tableName, QList<QHash<QString, QString>> &data, QString sqlWhere) {
...
//获取数据
bool mydatabase::getData(QString tableName, QHash<QString, QString> columndata, QList<QHash<QString, QString>> &data, QString sqlWhere) {
    QString colunmStr;
...
//增加
bool mydatabase::addData(QString tableName, QHash<QString, QString> data) {
    if(data.isEmpty()) {
        return false;
    }
...
//删除
bool mydatabase::delData(QString tableName, QString sqlWhere) {
...
//修改
bool mydatabase::updateData( QString tableName, QHash<QString, QString> data, QString sqlWhere) {
