...
{
    ui->setupUi(this);
    setAttribute(Qt::WA_DeleteOnClose);  //当窗口关闭时自动删除窗口对象，释放内存
    this->setWindowModality(Qt::ApplicationModal); //使当前窗口成为应用程序级别的模态窗口，阻塞其他所有窗口的输入
    //信号和槽函数的连接
...
//            QMessageBox::information(this, tr("消息"), tr("房间号仅限数字！"));
//                        return;
        }
    }
    GameInterface *g = new GameInterface();
